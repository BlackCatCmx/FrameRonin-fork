...
export function removeWatermarkReverseAlpha(
  ...
  for (let py = y1; py < y2; py++) {
    for (let px = x1; px < x2; px++) {
      ...
      const alpha = Math.min(alphaMap[alphaIdx] ?? 0, maxAlpha)
